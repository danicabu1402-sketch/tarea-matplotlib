...
# 3. Hacer la transpuesta y crear dos copias: una con order 'F' y otra con order 'C'
matriz_T = matriz.T
matriz_F = np.copy(np.reshape(matriz_T, matriz_T.shape, order='F'))
matriz_C = np.copy(np.reshape(matriz_T, matriz_T.shape, order='C'))

# 4. Generar un subplot con 6 paneles “a mano” con proporciones diferentes
...
# 5. En cada panel un gráfico diferente, usando cada fila de la matriz
...
# 6. Añadir labels, leyendas y cuadrículas
